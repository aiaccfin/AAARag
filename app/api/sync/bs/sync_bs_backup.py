...
async def bs_upload(
    ...
    try:
        ...
        # Save file
        uuid_name, uuid_name1, uuid_name2 = await s_file_system.save_upload_file_2_uuid(oUploadFile)
        logger.info(f" ------------ File saved with UUID: {uuid_name}, File Name: {uuid_name1} or {uuid_name2}")
        ...
        # background_tasks.add_task(process_file, oUploadFile, pdf_folder, pdf_file_name, uuid_name, db)

        return {
            "status: ": "success",
            "unique id: ": uuid_name,
            "uuid_name1": uuid_name1,
            "uuid_name2": uuid_name2,
        }
