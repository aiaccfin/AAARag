...
from app.llm   import ai_tax
from app.utils import pdf264

def tax_imgpdf(in_file_name: str, out_folder: str):
    try:
        # Convert PDF to text using your Ghostscript function
        b64_image   = pdf264.pdf_to_one_b64(in_file_name)
        ai_res = tax_b64_one_2_json(b64_image, in_file_name, out_folder)
...
def tax_textpdf(oUploadFile: UploadFile, pdf_name, pdf_folder):
    pdf_reader = PdfReader(oUploadFile.file)
    
    text = "\n".join(page.extract_text() for page in pdf_reader.pages)
    output_json = text.split("\n")
    
    ai_res = ai_tax.tax_ai_txt(output_json)
...
def tax_b64_one_2_json(base64_image, original_filename, output_directory):
    output_directory = os.path.abspath(output_directory)
    os.makedirs(output_directory, exist_ok=True)

    try:
        print("Processing image...")
        one_page_json_string = ai_tax.tax_ai_b64(base64_image=base64_image)
...
def tax_image(in_file_name: str, out_folder: str):
    try:
        # Convert PDF to text using your Ghostscript function
        b64_image   = pdf264.pdf_to_one_b64(in_file_name)
        ai_res = tax_b64_one_2_json(b64_image, in_file_name, out_folder)
...
def tax_text(in_file_name: str, out_folder: str):
    try:
        # Convert PDF to text using your Ghostscript function
        b64_image   = pdf264.pdf_to_one_b64(in_file_name)
        ai_res = tax_b64_one_2_json(b64_image, in_file_name, out_folder)
