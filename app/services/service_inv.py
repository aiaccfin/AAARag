...
from app.celery.celery_worker import embed_invoice_task
...
class InvService:
    ...
    def list_inv_rec(
        ...
        items, total = inv_repository.get_all(
            session=session,
            page=page,
            page_size=page_size,
            status=status,
    ...
    def create_inv_rec(
            ...
            tenant_id: uuid.UUID,
            doc_type: str,            
            session: Session) -> InvoiceDB:
        inv_data = data.dict()
        inv_data['inv_rec'] = doc_type
        
        new_inv_rec = InvoiceDB(**inv_data, tenant_id=tenant_id)
        inv_repository.save_invoice(new_inv_rec, session)
        
        logger.info("Enqueueing Celery task for embedding...")
        result = embed_invoice_task.delay(str(new_inv_rec.id))
    ...
    def get_next_receipt_number_for_me(self, db: Session, tenant_id: uuid.UUID) -> int:
        stmt = text("SELECT next_receipt_number(:tenant_id)")
        result = db.exec(stmt, params={"tenant_id": str(tenant_id)})
        db.commit()
        return result.scalar_one()

    def get_next_credits_number_for_me(self, db: Session, tenant_id: uuid.UUID) -> int:
        stmt = text("SELECT next_credit_number(:tenant_id)")
        result = db.exec(stmt, params={"tenant_id": str(tenant_id)})
        db.commit()
        return result.scalar_one()
    ...
    def list_inv(self, inv_rec, session: Session):
        return inv_repository.get_all(inv_rec, session)
